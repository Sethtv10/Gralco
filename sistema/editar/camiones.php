...
if(isset($_POST['idpol']))
{
	...
	//Editar la poliza y la tabla camiones
			...
			$sqlnvo.= "carga='".$_POST['tipocarga']."'";
			$sqlnvo.= " WHERE idpol='".$_POST['idpol']."'";
			$resnvo = mysql_query($sqlnvo) or die (mysql_error());
			$mensaje="Se guardaron los cambios.";
			$camposbit="usuario,accion,poliza,fecha,hora";
			$valorbit = "'".$_SESSION['usera']."','Edito poliza camiones','".$_POST['poliza']."/".$_POST['idpol']."','".date("j")."/".date("n")."/".date("Y")."','".date("g:i a")."'";
...
if(mysql_num_rows($res)>0)
{
	while(list($id,$numpoliza,$tipopoliza,$contratante,$datos,$deldia,$delmes,$delanio,$aldia,$almes,$alanio,$expdia,$expmes,$expanio,$compania,$moneda,$formapago,$primaneta,$gastoexp,$pagofracc,$derpol,$iva,$importetotal,$agente,$subagente,$polizaant,$status,$motivo,$comsub,$diasub,$messub,$aniosub,$notacredito)=mysql_fetch_array($res))
	{
		...
		$aniosubn=$aniosub;
		$notan=$notacredito;
		$sqle="SELECT * FROM camiones WHERE idpol='".$idn."'";
		$rese=mysql_query($sqle);
		if(mysql_num_rows($rese)>0)
		{
			while(list($idq,$idpolq,$marcaq,$motorq,$serieq,$placasq,$modeloq,$coberturaq,$origenq,$cargaq)=mysql_fetch_array($rese))
			{
				$idauto=$idq;
				$marcah=$marcaq;
				$motorh=$motorq;
...
<div id="contenido">
		<p>Editar póliza camiones</p>
			<form id="formRegistro" action="camiones.php<?php  echo "?".$_SERVER['QUERY_STRING']; ?>" method="post">
			...
			<table cellpadding="0" cellspacing="0" border="1" align="center" width="387">
                ...
                <tr>
                    ...
                    <td><select name="tipocarga" size="1" >
                        <option <?php  if ($cargah=="No peligrosa") { ?> selected="selected" <?php  } ?> value="No peligrosa">No peligrosa</option>
                        <option <?php  if ($cargah=="Peligrosa") { ?> selected="selected" <?php  } ?> value="Peligrosa">Peligrosa</option>
                        </select></td>
                </tr>
				 <tr align="center">
        		...
        		<tr align="center">
                    <td colspan="5" align="center"><div id="btn">
                      <input type="submit" name="submit" value="Editar póliza" />
						<input type="hidden" name="tipopol" value="camiones" />
