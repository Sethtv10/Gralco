...
if (mysql_num_rows($res)>0)
 {
	while(list($id,$numpoliza,$tipopoliza)=mysql_fetch_array($res))
	{
		if($tipopoliza=="vida")
		{
			header("LOCATION: paysub/vida.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="gm")
		{
			header("LOCATION: paysub/gm.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="daños")
		{
			header("LOCATION: paysub/danios.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="autos")
		{
			header("LOCATION: paysub/autos.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="camiones")
		{
			header("LOCATION: paysub/camiones.php?num=$numpoliza");
			exit;
		}
	}
 }
...
<div id="contenido">
		<p>Pagar póliza subagente</p>
			<form id="formRegistro" action="paysub.php" method="post">
