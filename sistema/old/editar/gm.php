...
if(isset($_POST['idpol']))
{
	...
	//Editar la poliza y la tabla autos
			...
			$sqlnvo = "UPDATE gm SET ";
			$sqlnvo.= "plan='".$_POST['plan']."',";
			$sqlnvo.= "titular='".$_POST['titular']."',";
			$sqlnvo.= "dependientes='".$_POST['dependientes']."'";
			$sqlnvo.= " WHERE idpol='".$_POST['idpol']."'";
			$resnvo = mysql_query($sqlnvo) or die (mysql_error());
			$mensaje="Se guardaron los cambios.";
			$camposbit="usuario,accion,poliza,fecha,hora";
			$valorbit = "'".$_SESSION['usera']."','Edito poliza gm','".$_POST['poliza']."/".$_POST['idpol']."','".date("j")."/".date("n")."/".date("Y")."','".date("g:i a")."'";
...
<div id="contenido">
		<p>Editar póliza gastos médicos</p>
			<form id="formRegistro" action="gm.php<? echo "?".$_SERVER['QUERY_STRING']; ?>" method="post">
			...
			<table cellpadding="0" cellspacing="0" border="1" align="center" width="387">
                ...
                <tr>
                    ...
                    <td><input type="text" size="30" name="titular" id="titular" autocomplete="off" value="<? echo $titularh; ?>" /></td>
                </tr>
                <tr>
                    <td><div id="etiqueta">
                      <label>Dependiente(s):</label></div></td>
                    <td><textarea name="dependientes" rows="25" cols="5"><? echo $dependientesh; ?></textarea></td>
                </tr>
				 <tr align="center">
