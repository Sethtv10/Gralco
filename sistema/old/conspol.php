...
$sql="SELECT id,numpoliza,tipopoliza FROM poliza WHERE numpoliza='".$numpol."' LIMIT 1";
$res=mysql_query($sql) or die (mysql_error());
if (mysql_num_rows($res)>0)
 {
	 $camposbit="usuario,accion,poliza,fecha,hora";
	 $valorbit = "'".$_SESSION['usera']."','Consulto poliza','".$numpol."','".date("j")."/".date("n")."/".date("Y")."','".date("g:i a")."'";
	...
	while(list($id,$numpoliza,$tipopoliza)=mysql_fetch_array($res))
	{
		if($tipopoliza=="vida")
		{
			header("LOCATION: consultar/vida.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="gm")
		{
			header("LOCATION: consultar/gm.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="daños")
		{
			header("LOCATION: consultar/danios.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="autos")
		{
			header("LOCATION: consultar/autos.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="camiones")
		{
			header("LOCATION: consultar/camiones.php?num=$numpoliza");
			exit;
		}
	}
 }
...
<div id="contenido">
		<p>Consultar póliza</p>
			<form id="formRegistro" action="conspol.php" method="post">
