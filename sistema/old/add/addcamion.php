...
if (isset($_POST['submit']))
 {
	...
	if ($valmarca!="")
	{
	$sqlmarca="SELECT nombre FROM marcacam WHERE nombre='".$valmarca."'";
	$resmarca=mysql_query($sqlmarca) or die (mysql_error());
		if (mysql_num_rows($resmarca)==0)
		  {
			$sqlmarca2 = "INSERT INTO `seguros_gralco`.`marcacam` (`id` ,`nombre` ) VALUES (NULL , '".$valmarca."')";
			$resmarca2 = mysql_query($sqlmarca2) or die (mysql_error());
		  }
	}
	$camposbit="usuario,accion,poliza,fecha,hora";
	 $valorbit = "'".$_SESSION['usera']."','Agrego poliza camiones','".$_POST['poliza']."/".$ultimoid."','".date("j")."/".date("n")."/".date("Y")."','".date("g:i a")."'";
