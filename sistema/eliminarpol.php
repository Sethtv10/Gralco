...
if (mysql_num_rows($res)>0)
 {
	while(list($id,$numpoliza,$tipopoliza)=mysql_fetch_array($res))
	{
		if($tipopoliza=="vida")
		{
			header("LOCATION: eliminar/vida.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="gm")
		{
			header("LOCATION: eliminar/gm.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="daños")
		{
			header("LOCATION: eliminar/danios.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="autos")
		{
			header("LOCATION: eliminar/autos.php?num=$numpoliza");
			exit;
		}
		if($tipopoliza=="camiones")
		{
			header("LOCATION: eliminar/camiones.php?num=$numpoliza");
			exit;
		}
	}
 }
...
<div id="contenido">
		<p>Eliminar póliza</p>
			<form id="formRegistro" action="eliminarpol.php" method="post">
				...
				<tr valign="top">
					<td width="950">
						<table width="430" align="center" border="0" cellspacing="0" cellpadding="0">
								<tr valign="top">
									<td width="111"><div id="etiqueta">
						      <label>Número de póliza:</label></div></td>
									<td width="319"><input type="text" name="txtpoliza" id="txtpoliza" autocomplete="off" size="55" />
